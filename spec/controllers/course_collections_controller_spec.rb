...
describe CourseCollectionsController do
  ...
  let(:collection) { FactoryGirl.create(:course_collection, user: admin) }
  ...
  describe "for a non-admin user" do
    ...
    describe "PATCH 'append_to'" do
      it "denies access" do
        expect{
          patch 'append_to', id: collection.pid, pid: 'pid:1'
        }.to raise_error(CanCan::AccessDenied)
      end
    end
  end
  ...
  describe "an admin user" do
    ...
    describe "POST 'create'" do
      it "creates a course collection" do
        ...
        expect(assigns[:curated_collection].edit_users).to eq [admin.user_key]
    ...
    describe "PATCH 'append_to'" do
      it "returns http success" do
        patch 'append_to', id: collection.pid, pid: image.pid
        expect(response).to be_successful
